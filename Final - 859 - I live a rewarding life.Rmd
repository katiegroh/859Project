---
title: "859 Project - Life Rewarding Score Predicting GPA"
author: Katie Groh
date:
...
```{r}
featurePlot(x = smart_food[, ], y = smart_food$life_rewarding, 
            plot = "scatter",
...
```{r}
rewarding_life = smart_food %>% mutate(goodlife = if_else(life_rewarding>5, "good", "bad")) %>% 
  mutate(goodlife = as.factor(goodlife))
ggplot(rewarding_life, aes(goodlife, life_rewarding, colour = goodlife, fill = goodlife))+
  geom_point(size = 1, alpha = .7, position = position_jitter(height = 0.1))+
  #labs(x = "Discretized wine quality", y = "Rated wine quality")+
  theme(legend.position = "none")
smart_food = rewarding_life %>% select(-life_rewarding)
```
...
# for (i in colnames(rewarding_life[, sapply(rewarding_life, is.factor)])){
#     for (level in unique(rewarding_life[, i])){
#         rewarding_life[paste(i, level, sep = "_")] = 
#             as.integer(ifelse(rewarding_life[, i] == level, 1, -1))
#     }
# }
...
predicted.df = cbind(xgb.pred, rewarding_life[-inTrain, ])
library(ggforce)
predicted_health.plot = 
  ggplot(predicted.df, aes(as.factor(life_rewarding), good, colour = goodlife)) + 
  geom_sina() +
  labs(title = "Prediction of good and bad health", 
       subtitle = "Extreme gradient boosting predictions (xgboost)",
...
explanation <- explain(testScaled, explainer, n_labels = 1, n_features = 4)
# Only showing part of output for better printings
explanation[,]
...
featurePlot(explanation, ncol = 1)
